...
# --- 2. NEW FUNCTION TO LOAD CSS ---
def load_css(file_name):
    try:
        with open(file_name) as f:
            st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
    except FileNotFoundError:
        st.error(f"CSS file {file_name} not found. Please check path.")
...
# ---
with st.sidebar:
    st.markdown("### 🧠 MemoryPalAI Status")
    st.info("Pinecone retrieval & style-aware learning pipeline.")
    ...
    if pinecone_db and st.button("🔥 Clear Pinecone Index"):
        with st.spinner("Deleting all vectors from Pinecone..."):
            try:
                pinecone_db.index.delete(delete_all=True, namespace="memorypal")
...
def content_already_indexed(db, raw_text):
    ...
    if hasattr(db, "has_content"):
        try: return db.has_content(raw_text)
...
tabs = st.tabs(["📤 Upload & Index", "💬 Retrieve & Learn", "🕓 History"])
...
with tabs[0]:
    ...
    if st.button("🔁 Process & Index", key="process_index"):
        if not uploaded_file and not url_text:
            ...
            with st.spinner("1/4 - Extracting text..."):
                if uploaded_file:
            ...
            if not raw_text or not raw_text.strip():
                ...
                with st.spinner("2/4 - Detecting style and topic..."):
                    try:
                ...
                tone = sd.get("tone")
                tags = sd.get("tags", [])
                metadata = {"source": filename, "subject": subject, "style": style, "tone": tone, "tags": tags}

                if pinecone_db:
                    already = False
                    with st.spinner("3/4 - Checking for duplicates..."):
                        try:
                    ...
                    if already:
                        ...
                        with st.spinner(f"4/4 - Indexing {len(raw_text)} characters into Pinecone..."):
        ...
        if pinecone_db:
            try:
                ...
                if stats and isinstance(stats, dict):
                    ...
                    st.info("Could not retrieve index stats or index is empty.")
            except Exception as e:
                st.error(f"Could not retrieve Pinecone stats: {e}")
        
        st.write("**Session File History:**")
        uploaded_files = session.list_files()
        if uploaded_files:
            for f in uploaded_files:
                st.markdown(f"- {os.path.basename(f)}")
...
with tabs[1]:
    with st.container(border=True):
        ...
        st.write("Ask a question, get an answer, and receive a personalized plan.")
        ...
        run_button = st.button("🔎 Retrieve & Answer", key="retrieve_btn")

    # --- FIX: INDENTATION ERROR ---
    # This entire block of logic must be INSIDE the 'if run_button:'
    if run_button: 
        if not query or not query.strip():
            st.error("Please enter a question.")
        elif not pinecone_db:
...
"""
                ...
                for m in metas:
                    ...
                    if subj: 
                        ...
                        for t in (m.get("tags", []) or []):
                            # Filter out bad tags
                            if not t or not isinstance(t, str) or "streamlit_agraph" in t:
                                continue 
                ...
                if graph_data.get("nodes"):
                    ...
                    try:
                        render_knowledge_graph(graph_data) # This function now has st.container()
                    except Exception as e:
    ...
    # --- [QUIZ AND CONTEXT SECTION] ---
    # This block is now correctly indented to be outside the 'if run_button:'
                ...
                with st.form("quiz_form"):
                    for i, b in enumerate(blocks):
                        ...
                        for ln in lines[1:]:
                            ...
                            if m: opts.append(m.group(1).strip())
                            else:
                                if ln: opts.append(ln)
                        while len(opts) < 4: opts.append("N/A")
                        st.markdown(f"**{qline}**")
                        sel = st.radio(f"Select answer for Q{i+1}", options=opts, key=f"q_radio_{i}", label_visibility="collapsed")
                ...
                if submitted:
                    ...
                    with st.spinner("Evaluating quiz..."):
                        ...
                        if pct is not None and pct < 70:
                            ...
                            if pct is None: st.info("Could not detect score; skipping auto-revision.")
                            else: st.success(f"Good performance ({pct:.1f}%) — no revision needed.")
